...
class SynologyMoments(object):
    ...
    def get_albums(self, category):
        ...
        for k in range(len(albums['data']['list'])):
            if albums['data']['list'][k]['item_count'] > 0:
                ...
                albums['data']['list'][k]['url'] = base_url + urlencode(params) + kodi_header
            else:
    ...
    def get_photos(self, list_id):
        ...
        if list_id == 'recently_added':
            ...
            data[list_id.split('=')[0]] = list_id.split('=')[1] # Add the proper id (geocoding_id, person_id, album_id, ...)
